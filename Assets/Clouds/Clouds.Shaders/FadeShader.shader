Shader "CustomRenderTexture/FadeShader"
{
    Properties
    {
     ...
     SubShader
     {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target 
            {
                ...
                float RevealAmountTop = smoothstep(maskcol.r - _SmoothFeather,maskcol.r+ _SmoothFeather,_RevealValue);

                float RevealAmountTop_appear = smoothstep(maskcol.r - _SmoothFeather,maskcol.r + _SmoothFeather,_RevealValue + _FeartherValue);
                float RevealAmountDown = smoothstep(maskcol.r - _SmoothFeather,maskcol.r + _SmoothFeather,_RevealValue - _FeartherValue);
                float RevealDiff = RevealAmountTop_appear - RevealAmountDown;
                float3 finalcolor = lerp(textureColor.rgb,_EnrobeColor.rgb, RevealDiff);
                fixed4 finaltext = fixed4(finalcolor,textureColor.a * RevealAmountTop) * _AlphaValue;
                return finaltext;
            }
            ENDCG
        }
